...
public class FeudManager{
    ...
    private ArrayList<Player> team1List;
    private ArrayList<Player> team2List;
    ...
    public void addPlayer(Player p, Role teamrole){
        if (teamrole == this.team1Role)
            this.team1List.add(p);
        else if (teamrole == team2Role) {
            this.team2List.add(p);
        }
        else
            throw new InvalidParameterException("Provided team role does not match the currently registered roles.");
    }
    ...
    public Map<Player, Role> findPlayer(Player p, Role teamRole){
        ...
        if(teamRole == this.team1Role) {
            for (Player teammate : team1List) {
                if (teammate.getUsername().equals(p.getUsername()))
                    result.put(p, teamRole);
            }
        }
            ...
            throw new InvalidParameterException("The provided role is not currently registered in this game session.");
        return result;
    }
